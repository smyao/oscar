...
先读 `oscar_ascend_agent_start.md`，按其档案引用制度执行。`issue1_full_record.md` 实际为162条（G1–G34、#1–#128），按标题定位；旧标题条数与旧行号存在漂移。
...
- CV/history/window、production RuntimeProvider、物理页精确快照、MTP位置修正和全服务probe已有实现。CANN VM编译与官方CPU-debug通过；node93已通过75项store/merge原语真NPU探针，后续日志已进入TP4/MTP模型加载，最新已完成编译/初次profiling与KV预算，止于MTP元数据虚拟块副本（#128）；已支持none/align布局并区分物理存储与metadata128虚拟块，完整NPU/图/性能未验收，readiness或编译成功不能冒充这些证据。
