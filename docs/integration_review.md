...
| Native integrity comparison ran only after successful readiness (historical review finding) | Any earlier phase failure skipped the post-run mutation check | The prior revision moved comparison to `finally` and tested exit-code preservation. Superseded on 2026-09-22 by the user requirement to remove default environment/native-source/readiness audits; actual operator and service probes, owned-resource cleanup and failure reporting remain required |
